import {CursorPath, Mutation} from "@/core/history/type/history-record";
...
export function applyMutations(mutations: Mutation[]) {
    for (const mutation of mutations) {
        applyMutation(mutation, cloneAll(mutation.addedTemplate), mutation.newValue);
    }
}

export function revertMutations(mutations: Mutation[]) {
    for (const mutation of [...mutations].reverse()) {
        applyMutation(mutation, mutation.removedNodes, mutation.oldValue);
    }
}
...
function toMutation(record: MutationRecord): Mutation {
    return {
        ...
        newValue: null,
        addedTemplate: cloneAll(Array.from(record.addedNodes)),
        removedNodes: Array.from(record.removedNodes),
        live: Array.from(record.addedNodes),
    };
}

function cloneAll(nodes: Node[]): Node[] {
...
function applyMutation(mutation: Mutation, toInsert: Node[], value: string | null) {
    switch (mutation.type) {
        case "childList":
            mutation.live = replaceNodes(mutation.target, mutation.nextSibling, mutation.live, toInsert);
            break;
        case "characterData":
...
function replaceNodes(target: Node, nextSibling: Node | null, toRemove: Node[], toInsert: Node[]): Node[] {
